 var user = {};
    ...
    */
    function appendMessage(msg) {
      ...
      var humanDate = null;
           var html =   '<blockquote><h6>' + msg.username + ':</h6>' + 
              msg.message +
    ...
    function configureSocket(socket) {
      ...
      socket.on('all online users', function (users) {
        ...
        {
          cadena +=  users[i]._id+"*******"; 
          htmluser += '<li id="' + users[i]._id + '">' + users[i]._id + '</li>';
          
        }

        $('.online-userslist').html(htmluser);
        alert("Recivido del servidor all online users("+cadena+")");
      });
      ...
      socket.on('chat message', function (msg) {
        alert("Recivido del servidor chat message");
        appendMessage(msg);
      });
     ...
     */
      socket.on('latest messages', function (messages) {
        alert("Recivido del servidor latest message");
      ...
      socket.on('remove user', function (nuser) {
        alert("Recivido del servidor remove user >"+nuser._id);
        $('#' + nuser._id).remove();
      })
